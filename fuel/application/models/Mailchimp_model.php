...
class Mailchimp_model extends CI_Model {
    ...
    public function addEmailToList($emailAddress, $listName = null, $listId = null, $url) {
        ...
        //$listId is assigned the getListId();
        if(is_null($listId)) {
  ...
  //$listName and $url is passed to getListId method
    private function getListId($listName, $url) {
        ...
        //checking if the totalitems key in the array exists and if the value is greater than 0.
        if(isset($listResultArray->total_items) && $listResultArray->total_items > 0) {
            //loop through the total number of lists
            foreach($listResultArray->$lists as $list) {
              //comparing $listName passed to getListId() to $list-> name key.
        ...
        //return the listID which will be null or contain the value
        return $listID;
    }
    ...
    private function send($url, $args = null) {
        ...
        curl_close($ch);
        //json response is stored in data
        return $data;
    }
}
